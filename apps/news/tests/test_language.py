...
def test_custom_editor_visible_labels_are_spanish() -> None:
    ...
    for field_name in ["featured_image_caption", "og_image_caption"]:
        assert NewsPage._meta.get_field(field_name).verbose_name == "Pie de foto"
    for field_name in ["featured_image_alt_text", "og_image_alt_text"]:
        assert NewsPage._meta.get_field(field_name).verbose_name == "Texto alternativo"
    for field_name in ["featured_image_credit", "og_image_credit"]:
        assert NewsPage._meta.get_field(field_name).verbose_name == (
            "Crédito de imagen"
        )
